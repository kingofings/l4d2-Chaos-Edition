Action Event_ChargerCarryStartPost(Event event, const char[] name, bool dontBroadcast)
{
	float chance = GetRandomFloat(0.0, 1.0);
	if(g_karmaChargerChance.FloatValue == 1.0 || g_karmaChargerChance.FloatValue > chance)
	{
...
Action OnTakeDamageInsultToInjury(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
